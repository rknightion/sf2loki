...
def target(expr: str, ref: str = "A", legend: str | None = None, fmt: str = "time_series", instant: bool = False) -> dict:
...
panels.append(panel(
    "Events ingested rate by event type", "timeseries",
    [target(f'sum by (event_type) (rate(sf2loki_events_ingested_total{JOB}[5m]))', legend="{{event_type}}")],
    x=0, y=y, w=12, h=8, unit="ops",
...
panels.append(panel(
    "Events ingested rate by source", "timeseries",
    [target(f'sum by (source) (rate(sf2loki_events_ingested_total{JOB}[5m]))', legend="{{source}}")],
    x=12, y=y, w=12, h=8, unit="ops",
...
panels.append(panel(
    "ELF rows ingested rate", "timeseries",
    [target(f'sum by (event_type) (rate(sf2loki_eventlogfile_rows_ingested_total{JOB}[5m]))', legend="{{event_type}}")],
    x=12, y=y, w=12, h=8, unit="ops",
...
panels.append(panel(
    "API usage % used by limit", "bargauge",
    [target(
        f'100 * (1 - sf2loki_salesforce_limit_remaining{JOB} / sf2loki_salesforce_limit_max{JOB})',
        legend="{{limit_name}}", instant=True,
    )],
    x=0, y=y, w=12, h=8, unit="percent",
...
panels.append(panel(
    "Remaining vs max for key limits", "timeseries",
    [
        target(f'sf2loki_salesforce_limit_remaining{JOB}', ref="A", legend="remaining · {{limit_name}}"),
...
panels.append(panel(
    "Limits poll errors", "timeseries",
    [target(f'rate(sf2loki_salesforce_limits_poll_errors_total{JOB}[5m])', legend="poll errors/s")],
    x=0, y=y, w=24, h=6, unit="ops",
...
panels.append(panel(
    "Push outcomes rate by outcome", "timeseries",
    [target(f'sum by (outcome) (rate(sf2loki_loki_push_total{JOB}[5m]))', legend="{{outcome}}")],
    x=0, y=y, w=12, h=8, unit="ops",
    ...
    overrides=[
        ...
        {"matcher": {"id": "byName", "options": "retried"}, "properties": [{"id": "color", "value": {"mode": "fixed", "fixedColor": "yellow"}}]},
        {"matcher": {"id": "byName", "options": "dropped"}, "properties": [{"id": "color", "value": {"mode": "fixed", "fixedColor": "red"}}]},
    ],
))
panels.append(panel(
    "Push duration p50/p95/p99", "timeseries",
    [
        target(f'histogram_quantile(0.50, sum by (le) (rate(sf2loki_loki_push_duration_seconds_bucket{JOB}[5m])))', ref="A", legend="p50"),
        target(f'histogram_quantile(0.95, sum by (le) (rate(sf2loki_loki_push_duration_seconds_bucket{JOB}[5m])))', ref="B", legend="p95"),
        target(f'histogram_quantile(0.99, sum by (le) (rate(sf2loki_loki_push_duration_seconds_bucket{JOB}[5m])))', ref="C", legend="p99"),
    ],
...
panels.append(panel(
    "Lines truncated rate", "timeseries",
    [target(f'sum by (source) (rate(sf2loki_lines_truncated_total{JOB}[5m]))', legend="{{source}}")],
    x=12, y=y, w=12, h=8, unit="ops",
...
panels.append(panel(
    "Auth refreshes & errors rate", "timeseries",
    [
...
panels.append(panel(
    "Internal queue depth", "timeseries",
...
panels.append(panel(
    "Pub/Sub pending credits by topic", "timeseries",
...
panels.append(panel(
    "Pub/Sub reconnects rate", "timeseries",
    [target(f'sum by (topic) (rate(sf2loki_pubsub_reconnects_total{JOB}[5m]))', legend="{{topic}}")],
    x=12, y=y, w=12, h=8, unit="ops",
...
panels.append(panel(
    "Decode errors rate", "timeseries",
    [target(f'sum by (reason) (rate(sf2loki_decode_errors_total{JOB}[5m]))', legend="{{reason}}")],
    x=0, y=y, w=12, h=8, unit="ops",
...
panels.append(panel(
    "Replay/watermark age", "stat",
    [
        target(f'time() - sf2loki_last_replay_commit_timestamp_seconds{JOB}', ref="A", legend="replay commit age · {{topic}}"),
        target(f'time() - sf2loki_watermark_timestamp_seconds{JOB}', ref="B", legend="watermark age · {{source}}/{{object}}"),
    ],
    x=12, y=y, w=12, h=8, unit="s",
