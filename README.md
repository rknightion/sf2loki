...
- [Deployment › Kubernetes](docs/deployment/kubernetes.md) — the production Helm chart
  (OCI at `ghcr.io/rknightion/charts/sf2loki`) for the active-passive HA pair.
...
A production Helm chart (Deployment, ServiceAccount + the Role/RoleBinding above, probes, PDB,
resource requests/limits, optional NetworkPolicy/External-Secrets) lives in
[`deploy/helm/`](deploy/helm/) and is published as an OCI chart to
`ghcr.io/rknightion/charts/sf2loki`. Set `ha.enabled: true` + `replicaCount: 2` for the pair —
see [`deploy/helm/README.md`](deploy/helm/README.md) and the
...
> own ~5s closer budget, with margin — the [Helm chart](deploy/helm/) defaults to 40s
> (`terminationGracePeriodSeconds`); raise it if you raise `shutdown_grace`.
